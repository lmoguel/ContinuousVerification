...
def save_time_registers(sensor, tiempos_generacion):
    ...
    try:
        new_file = open('./tiempos/sc_generation_time_' + str(sensor) + '_10.csv', "w")
    ...
    while True:
        ...
        if ((i-1)%100 == 0):
            if(bandera_guardar == 0):
                print("Writing")
                f=open("catalogs/cc" + str(sensor) + "/sensor" + str(sensor) + "_" + str(contador) + "_10" + ".txt", "w")
                f.write(buffer)
                f.close()
                contador = contador + 1
                buffer = ""
