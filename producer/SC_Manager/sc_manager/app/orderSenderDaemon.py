...
DB_HOST = sys.argv[11]
DB_PORT = sys.argv[12]
DB_DATABASE = sys.argv[13]
organization_name = sys.argv[14]
building_block_id = sys.argv[15]
sc_ip = sys.argv[16]
...
def createImageinBC(iden, level, transaction_id, content_id, file_name, status): #QUERY TO THE BLOCKCHAIN NETWORK
    portBC = int(sc_ip) + 2
    ...
    if(raw):
        ...
        datosJson = r.json
        previousHash = datosJson['actual_hash']
        print("THE PREVIOUS HASH IS: " + previousHash)

    timeStamp = get_timestamp()
    data2send = {'id':iden, 'organizationName':organization_name, 'level':level, 'builDescription':building_block_id, 'tranId':transaction_id, 'conId':content_id, 'fileName':file_name, 'status':status, 'newPrevious_hash':previousHash, 'newActual_hash':content_id, 'newTimestamp':timeStamp}
    url = "http://" + ip_blockchain + ":" + str(portBC) + "/createImage"
    r = requests.post(url=url, json=data2send, headers=headers)
    print(str(r.text))
...
def save_time_registers():
    ...
    try:
        ...
        new_file = open('./tiempos/' + fi[2] + ".txt", 'w')
        for i in registration_times:
            new_file.write(i + ", " + str(registration_times[i]['inicio']) + ", " + str(registration_times[i]['final']) + ", " + str(registration_times[i]['total']) + "\n")
        new_file.close()
        print('The folder tiempos/' + fi[2] + '.csv has been created')
    except:
        print("Error en la creación de archivo tiempos/" + fi[2] + ".csv")
...
def sendOrder(content_id, transaction_id, file_name):
    global registration_times
    if(raw):
        iden = "Sample" + str(randrange(10000000000))
    else:
        id_iden_elements = data_manager.get_iden(content_id)
        iden = list(id_iden_elements)[0]
    id_order = get_random_string(length=80)
    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    data2send = {'id_order':id_order, 'status_o':status, 'transaction_id':transaction_id, 'content_id':content_id, 'logistic':logistic, 'file_name':file_name, 'iden':iden}
    url = "http://" + str(ip) + ":" + str(port) + "/add_orders"
    r = requests.post(url=url, json=data2send, headers=headers)
    start_registration_transaction = time.time()
    createImageinBC(iden, 'supply chain', transaction_id, content_id, file_name, 'Inicialized')
    final_registration_transaction = time.time()
    total_registration_transaction = final_registration_transaction - start_registration_transaction
    registration_times[file_name] = {'inicio':start_registration_transaction, 'final':final_registration_transaction, 'total':total_registration_transaction}
    print(str(r.text))
    ...
    while flag:
        ...
        if(information_step_1 == information_step_2 == information_step_3):
            ...
            if(theHash not in processed_files_hashes):
                ...
                sendOrder(theHash, transaction_id, file_information[0])
