...
def save_time_registers(sensor, tiempos_generacion):
    ...
    try:
        new_file = open('./tiempos/sc_generation_time_' + str(sensor) + '_1000.csv', "w")
    ...
    while True:
        ...
        if ((i-1)%100 == 0):
            if(bandera_guardar == 4):
                print("Writing")
                f=open("catalogs/cc" + str(sensor) + "/sensor" + str(sensor) + "_" + str(contador) + "_1000" + ".txt", "w")
                f.write(buffer)
                f.close()
                contador = contador + 1
                buffer = ""
