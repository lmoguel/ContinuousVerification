...
def sendOrder(content_id, transaction_id, file_name):
    id_order_elements = data_manager.get_id_order(content_id)
    id_order = id_order_elements[0]
    print("EL ID DE LA ORDEN ES: " + str(id_order))
    #id_order = get_random_string(length=80)
    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    data2send = {'id_order':id_order, 'status_o':status, 'transaction_id':transaction_id, 'content_id':content_id, 'logistic':logistic, 'file_name':file_name}
